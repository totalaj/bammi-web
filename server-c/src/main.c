...
static const char *s_listen_on = "ws://localhost:3000";
...
typedef struct msg_connect {
    msg_header_t header;
    uint8_t room_id[64];
    uint8_t player_id[64];
} msg_connect_t;

msg_header_t msg_header_parse(net_buf_t* buf) {
    msg_header_t result;
    net_read_ubyte(buf, &result.type, "type");
    return result;
}

void msg_header_write(net_buf_t* buf, msg_header_t* data) {
    net_write_ubyte(buf, data->type, "type");
}

msg_connect_t msg_connect_parse(net_buf_t* buf) {
    msg_connect_t result;
    
    for(int i = 0; i < 64; i++) {
        net_read_ubyte(buf, &result.room_id[i], "room_id");
    }
    for(int i = 0; i < 64; i++) {
        net_read_ubyte(buf, &result.player_id[i], "player_id");
    }

    return result;
}

void msg_connect_write(net_buf_t* buf, const msg_connect_t* data) {
    for(int i = 0; i < 64; i++) {
        net_write_ubyte(buf, data->room_id[i], "room_id");
    }
    for(int i = 0; i < 64; i++) {
        net_write_ubyte(buf, data->player_id[i], "player_id");
    }
}

void msg_parse(char* buf, size_t len) {
    net_buf_t net_buf = {0};
    memcpy(&net_buf.data, buf, len);
    msg_header_t header = msg_header_parse(&net_buf);
    switch(header.type) {
        case MSG_TYPE_CONNECT:
            {
                msg_connect_t msg_connect = msg_connect_parse(&net_buf);
                printf("room: %s, player: %s\n", msg_connect.room_id, msg_connect.player_id);
            }
            break;
        default:
...
void msg_receive(struct mg_connection *c, int ev, void *ev_data) {
    (void)ev;
